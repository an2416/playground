...
def robust_contact_reminder(
    ...
    for i in range(1, len(intervals)):
        ...
        weight = 0.1 if change_ratio < sudden_change_threshold else 1.0

        recent_error = abs(intervals[i] - ema_interval) / (ema_interval + 1e-6)
        local_alpha = min(alpha * 1.5, 0.5) if recent_error > 0.5 else max(alpha * 0.7, 0.05) if recent_error < 0.1 else alpha
    ...
    sufficiency_factor = min(max(interval_count / full_data_count, 0.3), 1.0)
    ...
    status = f"\U0001f514 z-score {z_score:.2f}: 연락 필요!" if z_score > z_threshold \
        else f"\U0001f552 EMA 주기 도달 (z-score {z_score:.2f})" if days_since_last >= ema_interval \
        else f"⏳ 아직 {abs(days_since_last - ema_interval):.0f}일 여유 있음 (z-score {z_score:.2f})"
...
for name, history in contacts_logs.items():
    result = robust_contact_reminder(name, history)
    reminder_results.append(result)
    reminder_results_raw[name] = result

st.subheader("연락자별 리마인더 상태")
df_reminder_display = pd.DataFrame(reminder_results)[[
    "Name", "Last Contact", "Next Expected", "Mean Interval", "EMA Interval", "z-Score", "Confidence", "Confidence_raw", "Status"
]]
...
for name, history in contacts_logs.items():
    ...
    dates_df = pd.DataFrame({'연락 날짜': [d.date() for d in history]})
    ...
    if fontprop:
        ...
        ax1.set_title(f"{name} 연락한 날짜", fontproperties=fontprop)
    else:
    ...
    if len(history) > 1:
        ...
        if fontprop:
            ...
            ax2.set_title(f"{name} 연락 날짜 간 interval days + EMA", fontproperties=fontprop)
        else:
...
if fontprop:
    ax.set_xlabel("연락 간격 (일)", fontproperties=fontprop)
    ax.set_ylabel("빈도", fontproperties=fontprop)
    ax.set_title("연락자별 연락 주기 분포", fontproperties=fontprop)
else:
